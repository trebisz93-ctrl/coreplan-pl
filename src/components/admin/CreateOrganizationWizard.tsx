...
import { Building2, User, CheckCircle2, ArrowRight, ArrowLeft, Loader2 } from 'lucide-react';
...
export const CreateOrganizationWizard = ({ open, onOpenChange }: Props) => {
  ...
  const resetForm = () => {
    ...
    setCreatedOrgId(null); setCreatedUserEmail('');
  };
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {step === 3 && <><CheckCircle2 className="h-5 w-5 text-primary" /> Podsumowanie</>}
          </DialogTitle>
        </DialogHeader>
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>E-mail</Label>
                <Input value={orgEmail} onChange={e => setOrgEmail(e.target.value)} placeholder="firma@email.pl" type="email" />
              </div>
              <div>
                <Label>Telefon</Label>
                <Input value={orgPhone} onChange={e => setOrgPhone(e.target.value)} placeholder="+48..." />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>NIP</Label>
                <Input value={orgNip} onChange={e => setOrgNip(e.target.value)} placeholder="Opcjonalnie" />
              </div>
              <div>
                <Label>Adres</Label>
                <Input value={orgAddress} onChange={e => setOrgAddress(e.target.value)} placeholder="Opcjonalnie" />
              </div>
            </div>
            <div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Imię *</Label>
                <Input value={userFirstName} onChange={e => setUserFirstName(e.target.value)} placeholder="Jan" />
              </div>
              <div>
                <Label>Nazwisko *</Label>
                <Input value={userLastName} onChange={e => setUserLastName(e.target.value)} placeholder="Kowalski" />
              </div>
            </div>
            <div>
              <Label>E-mail *</Label>
              <Input value={userEmail} onChange={e => setUserEmail(e.target.value)} placeholder="admin@firma.pl" type="email" />
            </div>
            <div>
              <Label>Hasło *</Label>
              <Input value={userPassword} onChange={e => setUserPassword(e.target.value)} placeholder="Min. 6 znaków" type="password" />
            </div>
          ...
          <div className="space-y-4 text-center py-4">
            ...
            <div className="bg-muted/50 rounded-lg p-4 text-left space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{createdUserEmail}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Status:</span>
                <Badge variant="secondary">W konfiguracji</Badge>
              </div>
            </div>
